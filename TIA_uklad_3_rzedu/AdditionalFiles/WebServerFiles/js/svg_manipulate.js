...
function UpdateCallback(obj, response, status) 
{
 ...
 var _Setpoint = htmlEncode(results[0]);
 var _Processvalue = htmlEncode(results[1]);
 var _PID_out = htmlEncode(results[2]);
ForceUpdate(_Setpoint, _Processvalue, _PID_out);
...
function ForceUpdate(_Setpoint, _Processvalue, _PID_out){
var svg = document.getElementById("model-svg");
if (document.getElementById("model-svg")) {
var svgDoc = svg.contentDocument;

var SetpointElement = svgDoc.getElementById("Setpoint_val");
SetpointElement.textContent = Math.round (_Setpoint * 100) / 100;
var ProcessvalueElement = svgDoc.getElementById("Processvalue_val");
ProcessvalueElement.textContent = Math.round (_Processvalue * 100) / 100;
