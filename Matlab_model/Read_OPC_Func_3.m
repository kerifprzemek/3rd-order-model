...
function [x] = Read_OPC_Func_3(y)
% variables
persistent init_Server;
persistent init_Nodes;
persistent uaClient3;
...
if init_Server == 0
    ...
    init_Server = 1;
    uaClient3 = opcua('192.168.1.100',4840);                
    connect(uaClient3);                                    
end
% define variable nodes in the server
if uaClient3.isConnected == 1 && init_Nodes == 0
    init_Nodes  = 1;
    % find DB "MatlabData" of the server
        DB_Node     = findNodeByName(uaClient3.Namespace, 'MatlabData', '-once');
    % find variables in the DB
        Var_Node_In = findNodeByName(DB_Node, 'rSetLevelRoundTank', '-once');
        Var_Node_Out = findNodeByName(DB_Node, 'lrActLevelRoundTank', '-once');

end
% read and write variables of the server
if uaClient3.isConnected == 1 && init_Nodes == 1
    % read value from server and store in "val"
    [val, ~, ~] = readValue(uaClient3, Var_Node_In);
    % assign input y of the function to variable
    writeValue(uaClient3, Var_Node_Out, y);                  
